...
#if defined(NDEBUG)
#error "Expedia cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Expedia
...
{
    RenameThread("expedia-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZEXPMinted()
{
...
void RecalculateZEXPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zEXP supply
...
bool RecalculateEXPSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert EXP to zEXP
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Expedia: We use certain sporks during IBD, so check to see if they are
